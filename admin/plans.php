                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    <input type="text" id="name" placeholder="Plan name" class="form-control">
                                </div>
                                <div class="form-group">
                                    <input type="text" id="time" placeholder="Attack time" class="form-control">
                                </div>
                                <div class="form-group">
                                    <input type="text" id="concurrents" placeholder="Concurrents" class="form-control">
                                </div>
                                <div class="form-group">
                                    <input type="text" id="price" placeholder="Price for plan" class="form-control">
                                </div>
                                <div class="form-group">
                                    <input type="text" id="length" placeholder="Subscription length in days" class="form-control">
                                </div>
                                <div class="form-group">
                                    <select class="custom-select" id="vip">
                                        <option value="">Has vip?</option>
                                        <option value="1">Yes</option>
                                        <option value="2">No</option>
                                    </select>
                                </div>
                                <div class="form-group">
                                    <button type="button" id="add_plan" class="btn btn-danger col-lg-12">Add plan to database</button>
                                </div>
                            </div>
                        </div>
                    </div>
                                ...
                                <table class="table table-striped table-bordered dt-responsive nowrap" style="width:100%;" id="plans">
                                    ...
                                    if ($user->hasPermission() === true) {
                                        ...
                                        while ($data = $mysql->fetch(PDO::FETCH_ASSOC)) {
                                            ...
                                            echo "<tr><td>{$name}</td><td>{$time}</td><td>{$concurrents}</td><td>{$price}</td><td>{$vip}</td><td>{$days}</td><td>{$action}</td></tr>";
                                        }
                                    }
    ...
    $(document).ready(function () {
            ...
            if (id) {
                ...
                $.ajax('inc/Requests/planRequest', {
                    ...
                    success:function (getResp) {
                        if(getResp === "plan_deleted") {
                            setTimeout(function () {
                                window.location = "plans";
                            }, 2500);
                            return toastr['success']("The plan has been deleted from the database", "MCSpam");
                        }
                        if (getResp === "failed_deleting"){
                            return toastr['error']("Something went wrong while deleting", "MCSpam");
                        }
                    }
                });
        ...
        $(document).on('click', '#add_plan', function () {
            ...
            if (name === "" || time === "" || concurrents === "" || price === "" || length === "" || vip === "") {
                return toastr['error']("Required input field was left empty");
            }
            $.ajax('inc/Requests/planRequest', {
                data: {
                    add: 1,
                ...
                method:"POST",
                success:function (getResp) {
                    if (getResp === "plan_added"){
                        setTimeout(function () {
                            window.location = "servers";
                        }, 2500);
                        return toastr['success']("The plan was added to the database", "MCSpam");
                    }
                    if(getResp === "failed_add"){
                        return toastr['error']("Failed to add the plan to the database", "MCSpam");
                    }
                }
            });
        })
