        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="page-title-box">
                    ...
                    <h4 class="page-title mb-2"><i class="mdi mdi-monitor-dashboard mr-2"></i>Administrator Dashboard
                    </h4>
                    <div class="">
                        <ol class="breadcrumb">
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    <input type="text" placeholder="Enter host to blacklist" id="host"
                                           class="form-control col-lg-12">
                                </div>
                                <div class="form-group">
                                    <button id="add" class="btn btn-danger col-lg-12">Add host to blacklists</button>
                                </div>
                            </div>
                        </div>
                    </div>
                                ...
                                <table class="table table-striped table-bordered dt-responsive nowrap"
                                    ...
                                    if ($user->hasPermission() === true) {
                                        ...
                                        while ($data = $mysql->fetch(PDO::FETCH_ASSOC)) {
                                            $ID = $data['ID'];
                                            $host = $data['host'];
                                            $action = '<button type="button" class="btn btn-danger col-lg-12" id="delete" data-id="' . $data['host'] . '">Delete</button>';

                                            echo "<tr><td>{$ID}</td><td>{$host}</td><td>{$action}</td></tr>";
                                        }
                                    }
...
<script>
    $(document).ready(function () {
        $(document).on('click', '#add', function () {
            ...
            if (host) {
                $.ajax('../inc/Requests/blacklistRequest', {
                    data: {
                        add: 1,
                        host: host
                    },
                    ...
                    success: function (getResp) {
                        if (getResp === "added_blacklist") {
                            setTimeout(function () {
                                window.location = "blacklist"
                            }, 2500);
                            return toastr['success']("The host has been added to the blacklist.", "MCSpam");
                        }
                        if (getResp === "not_added") {
                            return toastr['error']("Host was not added to the blacklist", "MCSpam");
                        }
                    }
                })
        ...
        $(document).on('click', '#delete', function () {
            ...
            if(host) {
                $.ajax('../inc/Requests/blacklistRequest', {
                    ...
                    method: "POST",
                    success:function (getResp) {
                        if(getResp === "blacklist_deleted") {
                            setTimeout(function () {
                               window.location = "blacklist";
                            }, 2500);
                            return toastr['success']("Server has been deleted from the blacklist");
                        }
                        if(getResp === "not_deleted"){
                            return toastr['error']("Failed to delete the blacklist", "MCspam");
                        }
                    }
                });
            }
