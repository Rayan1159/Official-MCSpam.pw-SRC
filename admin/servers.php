                        ...
                        <div class="card">
                            <div class="card-body">
                                <h4 class="mt-0 header-title">
                                    Add server to rotation pool
                                </h4>
                                <div class="form-group">
                                    <input type="text" id="api" placeholder="Enter API url" class="form-control">
                                </div>
                                <div class="form-group">
                                    <button class="col-lg-12 btn btn-info" type="button" id="add_server">Add server</button>
                                </div>
                            </div>
                        </div>
                    </div>
                                ...
                                <table class="table table-striped table-bordered dt-responsive nowrap" style="width:100%;" id="plans">
                                    ...
                                    if ($user->hasPermission() === true) {
                                        ...
                                        while ($data = $mysql->fetch(PDO::FETCH_ASSOC)) {
                                            ...
                                            echo "<tr><td>{$id}</td><td>{$api}</td><td>{$action}</td></tr>";
                                        }
                                    }
...
<script>
    $(document).ready(function () {
        $(document).on('click', '#add_server', function () {
            var api = $('#api').val();

            $.ajax('inc/Requests/serverRequest', {
                data:{
                    add: 1,
                    api: api
                },
                ...
                success:function (getResp) {
                    if (getResp === "server_added"){
                        setTimeout(function () {
                            window.location = "servers";
                        }, 2500);
                        return toastr['success']("The server has been added to the attack pool", "MCSpam")
                    }
                    if(getResp === "failed_add"){
                        return toastr['error']("Failed to add server", "MCSpam")
                    }
                }
            });
        });
        $(document).on('click', '#delete', function () {
            ...
            $.ajax('inc/Requests/serverRequest', {
                ...
                method:"POST",
                success:function (getResp) {
                    if(getResp === "server_deleted"){
                        setTimeout(function () {
                            window.location = "servers";
                        }, 2500);
                        return toastr['success']("The server has been deleted", "MCSpam");
                    }
                    if (getResp === "not_deleted") {
                        return toastr['error']("The server has not been deleted", "MCSpam");
                    }
                }
            })
        })
